...
import { Edit, Trash2, Calendar, Clock, DollarSign, Bell, Search } from 'lucide-react';
...
const ContasRecorrentesList = () => {
  ...
  const contasFiltradas = contas?.filter(conta => 
    ...
    (conta.descricao && conta.descricao.toLowerCase().includes(searchTerm.toLowerCase()))
  );
      ...
      {!contasFiltradas || contasFiltradas.length === 0 ? (
        <Card>
          <CardHeader>
            <CardTitle>Suas Contas Recorrentes</CardTitle>
            <CardDescription>
              {searchTerm ? 'Nenhuma conta encontrada com esse termo de busca' : 'Você ainda não cadastrou nenhuma conta recorrente'}
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-semibold">
            ...
            {searchTerm && ` (${contasFiltradas.length} encontradas)`}
          </h2>
